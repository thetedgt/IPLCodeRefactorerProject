...
bool Tokenizer::tokenize(std::vector<Token> &tokens)
{
    ...
    do
    {
        ...
        {
            std::cout << tokens.back().lexeme << " " << tokens.back().line << std::endl;
            std::cout<<" "<<token.lexeme <<" on line " << token.line <<std::endl;
            return false;
        }
...
std::string Tokenizer::parseString()
{
    ...
    while(true)
    {
        while(code[codeIndex] != bound && !isAtEOF() || (code[codeIndex] == '\\' && code[codeIndex+1] == 'n'))
        {
            nextSymbol();
        }
...
Token Tokenizer::produceToken(TokenType type)
{
    switch(type)
    {
        ...
        case TokenType::COMMA:                 return produceToken(type, ",");
...
Token Tokenizer::nextToken()
{
    ...
    switch(code[codeIndex])
    {
        ...
        case '|':
            ...
            {
                nextSymbol();
                return produceToken(TokenType::LOGIC_OR);
            }
            if(match('='))
            {
                nextSymbol();
    ...
    // number literals
    unsigned curPos = codeIndex;
    double num = parseNumber();
    if(isStateSuccess())
    {
        std::string lex(code + curPos, code + codeIndex);
        return produceToken(TokenType::NUMBER, lex, num);
    }
    ...
    //std::cout << code[codeIndex] << "invalid\n";
    return produceToken(TokenType::INVALID);
}
